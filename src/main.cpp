...
#if defined(NDEBUG)
# error "Oscintcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xafc0e1d81791cc660f6690b17f98a58346c4add1d2cdb3d598310e4024bac2fa");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Oscintcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Oscintcoin Signed Message:\n";
...
{
    // Oscintcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 60; // Oscintcoin: 1 minute
static const int64 nTargetSpacing = 1 * 60; // Oscintcoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Oscintcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Oscintcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xafc0e1d81791cc660f6690b17f98a58346c4add1d2cdb3d598310e4024bac2fa");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 13/Dec/2017 US at a Crossroad as It Confronts Turmoil in Honduras";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("048201610fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1513150130;
        ...
        assert(block.hashMerkleRoot == uint256("0xbbc6e9efdc6baa7fda35a4261932f157edb61f59aceb35b7b9b15a25aeb61fbc"));
...
unsigned char pchMessageStart[4] = { 0xcf, 0xd6, 0xd2, 0xb1 }; // Oscintcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OscintcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OscintcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static OscintcoinMiner(CWallet *pwallet)
{
    printf("OscintcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("oscintcoin-miner");
    ...
    try { loop {
        ...
        printf("Running OscintcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("OscintcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&OscintcoinMiner, pwallet));
}
